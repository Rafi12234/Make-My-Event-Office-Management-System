    ...
    <div className="fixed inset-0 z-[100] grid place-items-center bg-black/70 px-5 py-8 backdrop-blur-sm">
      <div className="w-full max-w-lg overflow-hidden rounded-[30px] border border-white/40 bg-white shadow-[0_30px_100px_rgba(35,16,45,0.35)]">
        ...
        <form onSubmit={handleSubmit} className="space-y-5 p-7 sm:p-9">
          <div>
            <label className="mb-2 block text-sm font-black text-black" htmlFor="employee-name">
              Employee name
            </label>
            <div className="relative">
              <UserRound className="absolute left-4 top-1/2 -translate-y-1/2 text-[#333333]" size={18} />
              <input
                id="employee-name"
                value={form.fullName}
                onChange={(event) => setForm((current) => ({ ...current, fullName: event.target.value }))}
                className="w-full rounded-2xl border border-[#d6d6d6] bg-white py-3.5 pl-12 pr-4 outline-none transition focus:border-[#333333] focus:ring-4 focus:ring-[#d6d6d6]/25"
          ...
          <div>
            <label className="mb-2 block text-sm font-black text-black" htmlFor="employee-email">
              Employee email
            </label>
            <div className="relative">
          ...
          <button className="w-full rounded-2xl bg-black px-6 py-4 text-sm font-black text-white shadow-xl shadow-black/20 transition hover:-translate-y-0.5 hover:bg-[#222222]">
